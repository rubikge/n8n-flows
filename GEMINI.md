...
Filters on `paths: workflows/**, scripts/**, .github/workflows/...` so doc-only changes don't trigger a deploy. `environment: production` on the prod workflow makes it visible in GitHub's Environments UI for adding manual approval gates later.

Dev is **one-way (dev → repo)** so workflows can be authored in the n8n-dev UI and saved to the repository:
...
Caveats:
- Do not push workflow JSON edits to a branch while you have uncommitted edits in the n8n-dev UI — the publish flow could clobber them.
